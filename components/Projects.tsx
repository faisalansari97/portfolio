...
const ProjectLink = ({ link }) => {
  const getIcon = () => {
    switch (link.type) {
      ...
      case 'ios': return <Smartphone size={14} />;
                ...
                {project.features && (
                   <ul className="mb-8 space-y-2">
                     {project.features.map((f, i) => (
                       <li key={i} className="text-sm text-slate-500 dark:text-slate-500 flex items-start gap-2.5">
                ...
                <div className="pt-6 border-t border-slate-100 dark:border-slate-800 mt-auto">
                   <div className="flex flex-wrap gap-2 mb-6">
                    {project.tech.map((t, i) => (
                      <span key={i} className="text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700/50">
                        {t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.links.length > 0 ? (
                      project.links.map((link, i) => <ProjectLink key={i} link={link} />)
                    ) : (
                      <span className="text-xs font-medium text-slate-400 italic px-2 py-1">Under Development / Internal</span>
                    )}
                  </div>
                </div>
              </div>
